...
// cari di database
$sql = "SELECT * from tb_katadasar where katadasar ='$kata'"; 
//echo $sql.'<br/>';
...
/*---function Del_Infix($kata){
if(preg_match('/^[[:alpha:]]+(er)/$',$kata)){ 
$__kata = preg_replace('/^(er)/$','',$kata);
if(cekKamus($__kata)){
return $__kata; // Jika ada balik
}
}
$__kata__ = Del_Infix($__kata);
if(cekKamus($__kata__)){
return $__kata__;
}
if(preg_match('/^[[:alpha:]]+(er)/$',$kata) == FALSE){
return $kataAsal;
}
}
